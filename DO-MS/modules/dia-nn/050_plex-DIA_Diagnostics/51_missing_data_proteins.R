init <- function() {
  ...
  type <- 'plot'
  box_title <- 'Missing Data,  Protein Level'
  ...
  .jaccard_by_label <- function(dataframe, experiment){
    ...
    for(i in 1:nrow(idx)){
      ...
      precursors_first = unique(precursors_first[["Protein.Ids"]])
      
      precursors_second = dataframe[dataframe$Label == second_label, ]
      precursors_second = unique(precursors_second[["Protein.Ids"]])
  ...
  .plotdata <- function(data, input) {
    plotdata <- data()[['report']][,c('Raw.file', 'Ms1.Area', 'Precursor.Id','Label' ,'Protein.Ids','Proteotypic')]
    ...
    plotdata <- plotdata[plotdata$Proteotypic==1,]
