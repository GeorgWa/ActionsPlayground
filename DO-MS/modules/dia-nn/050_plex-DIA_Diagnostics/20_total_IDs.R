init <- function() {
  ...
  .plotdata <- function(data, input) {
    ...
    plotdata <- plotdata %>% 
      mutate(Type=recode(Type, 
                         Identifications.T = "trans. q-val > 0.01", 
                         Identifications.O = "trans. q-val <= 0.01"))
    
    levels_plot = c("trans. q-val > 0.01","trans. q-val <= 0.01")
    plotdata <- within(plotdata, 
  ...
  .plot <- function(data, input) {
    ...
    ggplot(plotdata, aes(x=Label, y=Identifications, fill=Label, colour=Label,  alpha=Type)) +
      ...
      scale_fill_manual(values = c(custom_colors[[6]],custom_colors[[1]],custom_colors[[2]],custom_colors[[3]],custom_colors[[6]]), guide=FALSE)+
      scale_color_manual(values = c(custom_colors[[6]],custom_colors[[1]],custom_colors[[2]],custom_colors[[3]],custom_colors[[6]]), guide=FALSE)
