init <- function() {
  ...
  type <- 'plot'
  box_title <- 'Miscleavage Rate (R), PEP < 0.01'
  help_text <- 'Plotting frequency of arginine miscleavages in confidently identified precursors.'
  source_file <- 'report'
  ...
  .plotdata <- function(data, input) {
    ...
    plotdata$Missed.cleavages = sapply(plotdata$Stripped.Sequence, .get_internal_occurence, char="R")
