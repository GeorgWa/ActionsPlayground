init <- function() {
  ...
  type <- 'plot'
  box_title <- 'MS1 Intensity for Intersected Precursors, summed over all Channels.'
  help_text <- 'Plotting the MS1 intensity for all precursors summed over all channels. Only intersected precursors present in all loaded experiments are shown.'
  source_file <- 'report'
  ...
  .plotdata <- function(data, input) {
    plotdata <- data()[['report']][,c('Raw.file', 'Ms1.Area','Precursor.Id')]
